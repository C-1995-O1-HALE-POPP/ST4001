...
This thesis presents the Long-Horizon Reasoning Task of \textbf{LongMedBench}, a benchmark component constructed from MIMIC-IV \cite{PhysioNet-mimiciv-3.1} EHR data that evaluates whether LLM agents can reason over longitudinal clinical history before selecting next-step clinical decisions. Our work bridges two previously disconnected lines of research: the design of long-horizon interaction benchmarks from the general AI domain, and the evaluation of clinical reasoning from real-world EHR data. Through a three-tier memory architecture and systematic ablation of memory strategies, we investigate a central question: given long-context windows, RAG, and agent memory systems, how well can contemporary LLMs integrate longitudinal clinical history to inform next-step decisions?

\section{Medical Benchmarks for Clinical AI}

\begin{table}[ht]
    \centering
    \caption{Medical AI benchmarks across EHR grounding, agent interaction, longitudinal design, and memory evaluation. \textit{Legend:} EHR = real electronic health record data. Agent = interactive agent-based evaluation. Memory Study = systematic memory architecture ablation.}
    \label{tab:med_bench}
...
\begin{table}[ht]
    \centering
    \caption{Representative memory-augmented architectures for long-horizon LLM agents. \textit{Legend:} Dynamic = memory state evolves across interactions.}
    \label{tab:memory}
    {\fontsize{8}{9.5}\selectfont
    \setlength{\tabcolsep}{3pt}
    \renewcommand{\arraystretch}{1.1}
...
Specifically, focusing on the Long-Horizon Reasoning Task within LongMedBench, this thesis investigates three questions:
...
\begin{enumerate}
    ...
    \item \textbf{Three-Tier Memory Architecture.} A memory framework operating at three granularities, enabling systematic ablation of memory effects on decision quality: Note Memory $\mathcal{M}^{(i,j)}_N$ (coarse-grained visit summaries), Event Memory $\mathcal{M}^{(i,j)}_E$ (fine-grained structured event sequences), and Context Memory $\mathcal{M}_C^{(j,T)}$ (immediate visit context).
    
    \item \textbf{Long-Horizon Reasoning Task.} An interactive evaluation protocol where the agent receives a partial patient trajectory through controlled memory, writes an explicit rationale, and then predicts the next clinical action or decision from predefined options, with ground truth extracted from actual clinical records.
    
    \item \textbf{Empirical Finding.} Through experiments across GPT-5-mini \cite{gpt52}, DeepSeek-v3.2 \cite{deepseekai2025deepseekv32}, and Qwen-Turbo \cite{qwen3} comparing Naive Long-Context LLM, RAG, and Mem0 \cite{mem0_reference} strategies, we find that memory augmentation provides no significant improvement for longitudinal clinical decision-making. Decision accuracy depends primarily on immediate context, revealing a retrieval-reasoning gap that challenges current memory-centric approaches.
\end{enumerate}

\section{Scope and Structure}

This thesis focuses exclusively on the Long-Horizon Reasoning Task on LongMedBench. The Fact-based QA and Temporal Reasoning tasks are covered in collaborators' separate theses.

Chapter~2 presents the data processing pipeline, three-tier memory architecture, long-horizon reasoning task design, experimental setup, and results. Chapter~3 discusses the implications of our findings, acknowledges limitations, and outlines future research directions.
