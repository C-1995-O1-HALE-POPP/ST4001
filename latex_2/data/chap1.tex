...
Medical AI evaluation has evolved from static QA to interactive agent settings, as shown in Table~\ref{tab:med_bench}. Early benchmarks including MedBench \cite{2023_medbench} and PromptCBLUE \cite{2023_promptcblue} etc., tested LLm's knowledge recall from medical licensing exams. Second-generation work introduced real clinical data: EHRSQL \cite{lee2022ehrsql} pioneered text-to-SQL on structured EHR; FHIR-AgentBench \cite{2025_fhir_agentbench} extended this to interoperable FHIR standards; MedAlign \cite{2024_medalign} provided clinician-generated instruction-following tasks grounded in EHR data.
...
LongMedBench is positioned at the intersection of the fields above. On one hand, it inherits the domain grounding of medical benchmarks. On the other hand, it adopts the task design methodology of general long-horizon benchmarks.
...
Specifically, focusing on Long-Horizon Decision-Making tasks of LongMedBench, this thesis investigates three questions:
...
\begin{enumerate}
    ...
    \item \textbf{Three-Tier Memory Architecture.} A memory framework operating at three granularities enabling systematic ablation of memory granularity effects, including Note Memory (coarse-grained visit summaries), Event Memory (fine-grained structured event sequences), and Contextual Memory (immediate visit context).
