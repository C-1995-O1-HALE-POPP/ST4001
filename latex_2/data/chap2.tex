...
LongMedBench is constructed from MIMIC-IV v3.1 \cite{PhysioNet-mimiciv-3.1}, a publicly available de-identified database containing comprehensive clinical data for over 100,000 patients at Beth Israel Deaconess Medical Center (2008--2019). The database includes structured tables and unstructured clinical notes. To convert this static relational database into an interactive agent-compatible environment, we design a three-stage pipeline (Figure~\ref{fig:pipeline}).

\subsection{Patient and Event Filtering}

\paragraph{Event-Level Filtering.} We retain only laboratory results flagged as abnormal. Normal lab values carry limited clinical signal for decision-making and inflate context without benefit. Hager et al., demonstrated in MIMIC-CDM that presenting the model with only abnormal laboratory results generally improved diagnostic performance by allowing the model to focus more on abnormal, pathological signals \cite{hager2024evaluation}. This makes the evaluation more clinically realistic. Like real-world clinicians, the agent should reason from salient signals, not passively absorb all measurements.

\paragraph{Patient-Level Filtering.} We require complete admission and discharge records for all hospitalizations, and select patients with at least 15 recorded hospitalizations. This threshold yields trajectories with sufficient density for longitudinal reasoning evaluation (discussed in Chapter~3).
