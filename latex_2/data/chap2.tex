...
For a sampled ground-truth event $E_i^{GT}$ in the current visit $\mathcal{V}_i$ with timestamp $t_i^{GT}$, the working context is defined as $S=\mathcal{M}_C^{(i,t_i^{GT})}$, containing all interaction history available before the target event. To ensure the historical trajectory length, in the experiment we only take rounds where $n \ge 15$. The long-horizon memory is denoted as $M=\mathcal{M}^{(1,n)}_E$ or $M=\mathcal{M}^{(1,n)}_N$, depending on whether Event Memory or Note Memory is injected. At inference time, the agent receives $S$, an instructional prompt, $M$, and a multiple-choice query $Q$. The prompt requires a ReAct-inspired rationale-before-decision format: the agent first writes an explicit rationale or reasoning trace in the JSON \texttt{reason} field, then returns the selected answer list in the JSON \texttt{answer} field. The final answer is still a clinical decision or action, and the ground truth comes from the original EHR event stream. The three subtasks are defined as follows:
\begin{itemize}
